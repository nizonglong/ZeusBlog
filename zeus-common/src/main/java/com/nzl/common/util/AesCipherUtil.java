...
//        AesCipherUtil.encryptAESKey = "V2FuZzkyNjQ1NGRTQkFQSUpXVA==";
//    }
    ...
    public static String enCrypto(String str) {
        try {
            ...
            throw new ZeusUnauthorizedException("getInstance()方法异常:" + e.getMessage());
        } catch (UnsupportedEncodingException e) {
            logger.error("Base64加密异常:{}", e.getMessage());
            throw new ZeusUnauthorizedException("Base64加密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:{}", e.getMessage());
            throw new ZeusUnauthorizedException("初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("加密异常，密钥有误:{}", e.getMessage());
            throw new ZeusUnauthorizedException("加密异常，密钥有误:" + e.getMessage());
        }
    }
    ...
    public static String deCrypto(String str) {
        try {
            ...
            throw new ZeusUnauthorizedException("Base64解密异常:" + e.getMessage());
        } catch (InvalidKeyException e) {
            logger.error("初始化Cipher对象异常:{}", e.getMessage());
            throw new ZeusUnauthorizedException("初始化Cipher对象异常:" + e.getMessage());
        } catch (IllegalBlockSizeException | BadPaddingException e) {
            logger.error("解密异常，密钥有误:{}", e.getMessage());
            throw new ZeusUnauthorizedException("解密异常，密钥有误:" + e.getMessage());
        }
    }
}
