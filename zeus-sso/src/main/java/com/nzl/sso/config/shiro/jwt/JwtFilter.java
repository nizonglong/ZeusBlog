...
import com.nzl.common.util.JsonConvertUtil;
...
public class JwtFilter extends BasicHttpAuthenticationFilter {
    ...
    private void response401(ServletResponse response, String msg) {
        ...
        try (PrintWriter out = httpServletResponse.getWriter()) {
            String data = JsonConvertUtil.objectToJson(new ZeusResponseBean(HttpStatus.UNAUTHORIZED.value(), "无权访问(Unauthorized):" + msg, null));
            out.append(data);
        } catch (IOException e) {
            logger.error("直接返回Response信息出现IOException异常:{}", e.getMessage());
            throw new ZeusException("直接返回Response信息出现IOException异常:" + e.getMessage());
        }
    }
}
